 ...
 *
 * Usage:
 *   node lib/i18n/validate-translations.js           # strict mode (exit 2 on violations)
 *   node lib/i18n/validate-translations.js --warn    # warn mode (exit 0 always)
 *   node lib/i18n/validate-translations.js --strict  # explicit strict mode
 */
...
const strictMode = args.includes("--strict") || !warnMode

const LOCALES_DIR = path.join(__dirname, "locales")

// Load non-translatable whitelist
...
try {
  const whitelistConfig = JSON.parse(
    fs.readFileSync(path.join(__dirname, "non-translatable.json"), "utf8")
  )
  if (whitelistConfig.whitelist && Array.isArray(whitelistConfig.whitelist)) {
    whitelist = new Set(whitelistConfig.whitelist)
  }
} catch {
  // Silently fall back to default whitelist if config file not found
}
...
/**
 * Validate locale parity - ensure both locales have identical keys
 */
function validateLocaleParity(translations, locales) {
  const errors = []
  const keys = {}

  // Collect all keys per locale
  for (const locale of locales) {
    keys[locale] = new Set(Object.keys(flattenObject(translations[locale])))
  }

  // Get reference locale (en)
  const refLocale = locales.includes("en") ? "en" : locales[0]
  const refKeys = keys[refLocale]

  // Check all other locales match the reference
  for (const locale of locales) {
    if (locale === refLocale) continue

    const missingInTarget = [...refKeys].filter((k) => !keys[locale].has(k))
    const extraInTarget = [...keys[locale]].filter((k) => !refKeys.has(k))

    if (missingInTarget.length > 0) {
      errors.push({
        locale,
        type: "missing_keys",
        count: missingInTarget.length,
        keys: missingInTarget.slice(0, 5) // show first 5
      })
    }
    ...
    if (extraInTarget.length > 0) {
      errors.push({
        locale,
        type: "extra_keys",
        count: extraInTarget.length,
        keys: extraInTarget.slice(0, 5) // show first 5
      })
    }
  }
...
function validateTranslations() {
  ...
  console.log("\n📋 Checking locale parity...\n")
  const parityErrors = validateLocaleParity(translations, locales)
  if (parityErrors.length > 0) {
    console.warn("⚠️  Locale parity issues detected:\n")
    parityErrors.forEach((error) => {
      console.warn(`- ${error.locale}.json: ${error.type} (${error.count} total, showing first 5)`)
      error.keys.forEach((k) => console.warn(`    • ${k}`))
    })
    console.warn("")
    if (strictMode) {
      process.exit(1)
  ...
  console.log("\n🔗 Validating translation parameters...\n")
  let paramErrors = []
  for (const locale of locales) {
  ...
  if (paramErrors.length > 0) {
    console.warn("⚠️  Parameter issues detected:\n")
    paramErrors.slice(0, 10).forEach((error) => {
      console.warn(`- ${error.locale}: ${error.key} has ${error.issue} '${error.param}'`)
    })
    if (paramErrors.length > 10) {
      console.warn(`... and ${paramErrors.length - 10} more`)
    }
    console.warn("")
    if (strictMode) {
      process.exit(1)
    ...
    console.log("✓ No parameter issues detected")
  }
  ...
  console.log("\n🔎 Scanning source files for hardcoded UI strings (AST-based)...\n")
    ...
    function visit(node) {
      ...
      if (ts.isCallExpression(node)) {
        ...
        if (ts.isPropertyAccessExpression(expr)) {
          ...
          if (
            left === "toast" &&
            ["success", "error", "message", "info", "warning"].includes(right)
          ) {
            const arg = args[0]
            if (arg && (ts.isStringLiteral(arg) || ts.isNoSubstitutionTemplateLiteral(arg)))
              addIfCandidate(arg.text, arg)
          ...
          if (zodMethods.has(right)) {
            ...
            if (
              lastArg &&
              (ts.isStringLiteral(lastArg) || ts.isNoSubstitutionTemplateLiteral(lastArg))
            ) {
          ...
          if (right === "replace" && args.length >= 2) {
            ...
            if (
              valArg &&
              (ts.isStringLiteral(valArg) || ts.isNoSubstitutionTemplateLiteral(valArg))
            ) {
        ...
        if (uiKeys.has(name) && node.initializer) {
          ...
          if (ts.isStringLiteral(init) || ts.isNoSubstitutionTemplateLiteral(init)) {
            ...
            if (name === "name" || name === "content") {
              ...
              if (txt.includes(" ") || /[a-z]/.test(txt)) {
                // likely a display name if it has spaces or lowercase
                addIfCandidate(init.text, init)
              }
            } else {
              addIfCandidate(init.text, init)
  ...
  console.log("")
  console.log(
    warnMode ? "⚠️  Validation complete (warn mode - exit code 0)" : "✅ Validation passed"
  )
  process.exit(0)
}
