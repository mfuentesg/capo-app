...
CREATE POLICY "Owners and admins can update teams"
  ON teams FOR UPDATE
  USING (has_team_permission(teams.id, auth.uid(), 'admin'));

-- DELETE: Only owners can delete teams
CREATE POLICY "Only owners can delete teams"
  ON teams FOR DELETE
  USING (has_team_permission(teams.id, auth.uid(), 'owner'));
...
CREATE POLICY "Only system can insert team members"
  ON team_members FOR INSERT
  WITH CHECK (false);

-- UPDATE: Only owners/admins can change roles (via RPC)
CREATE POLICY "Only system can update team members"
  ON team_members FOR UPDATE
  USING (false);

-- DELETE: Only owners/admins can remove members (via RPC)
CREATE POLICY "Only system can delete team members"
...
CREATE POLICY "Team members can view invitations"
  ...
  USING (is_team_member(team_invitations.team_id, auth.uid()));

-- INSERT: Only owners/admins can create invitations
CREATE POLICY "Owners and admins can create invitations"
  ON team_invitations FOR INSERT
  WITH CHECK (
    has_team_permission(team_invitations.team_id, auth.uid(), 'admin')
    AND auth.uid() = invited_by
  );

-- UPDATE: System can update acceptance status (via RPC)
CREATE POLICY "Only system can update invitations"
...
CREATE POLICY "Owners and admins can delete invitations"
  ...
  USING (has_team_permission(team_invitations.team_id, auth.uid(), 'admin'));
...
CREATE POLICY "Team members can read team songs"
  ON songs FOR SELECT
  USING (
    team_id IS NOT NULL
...
CREATE POLICY "Users can create personal songs"
  ON songs FOR INSERT
  WITH CHECK (
    user_id = auth.uid()
    AND team_id IS NULL
...
CREATE POLICY "Team members can create team songs"
  ON songs FOR INSERT
  WITH CHECK (
    team_id IS NOT NULL
    AND user_id IS NULL
    AND created_by = auth.uid()
    AND has_team_permission(songs.team_id, auth.uid(), 'member')
  );
...
CREATE POLICY "Team members can update team songs"
  ON songs FOR UPDATE
  USING (
    team_id IS NOT NULL
    AND has_team_permission(songs.team_id, auth.uid(), 'member')
...
CREATE POLICY "Team members can delete team songs"
  ON songs FOR DELETE
  USING (
    team_id IS NOT NULL
    AND has_team_permission(songs.team_id, auth.uid(), 'admin')
  );
...
CREATE POLICY "Team members can read team playlists"
  ON playlists FOR SELECT
  USING (
    team_id IS NOT NULL
    AND is_team_member(playlists.team_id, auth.uid())
  );
...
CREATE POLICY "Team members can create team playlists"
  ON playlists FOR INSERT
  WITH CHECK (
    team_id IS NOT NULL
    AND user_id IS NULL
    AND created_by = auth.uid()
    AND has_team_permission(playlists.team_id, auth.uid(), 'member')
  );
...
CREATE POLICY "Team members can update team playlists"
  ON playlists FOR UPDATE
  USING (
    team_id IS NOT NULL
    AND has_team_permission(playlists.team_id, auth.uid(), 'member')
...
CREATE POLICY "Team members can delete team playlists"
  ON playlists FOR DELETE
  USING (
    team_id IS NOT NULL
    AND has_team_permission(playlists.team_id, auth.uid(), 'admin')
  );
...
CREATE POLICY "Users can read own playlist songs"
...
CREATE POLICY "Team members can read team playlist songs"
  ON playlist_songs FOR SELECT
  USING (
    EXISTS (
...
CREATE POLICY "Public playlist songs visible to all"
  ON playlist_songs FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM playlists
...
CREATE POLICY "Team members can insert into team playlists"
  ON playlist_songs FOR INSERT
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM playlists p
...
CREATE POLICY "Team members can update team playlist songs"
  ON playlist_songs FOR UPDATE
  USING (
    EXISTS (
      SELECT 1 FROM playlists p
...
CREATE POLICY "Team members can delete from team playlists"
  ON playlist_songs FOR DELETE
  USING (
    EXISTS (
      SELECT 1 FROM playlists p
...
CREATE POLICY "Team members can read team activity"
  ON activity_log FOR SELECT
  USING (
    team_id IS NOT NULL
    AND is_team_member(activity_log.team_id, auth.uid())
  );

-- INSERT: System only (via triggers)
CREATE POLICY "Only system can insert activity"
  ON activity_log FOR INSERT
  WITH CHECK (false);

-- UPDATE: System only
CREATE POLICY "Only system can update activity"
  ON activity_log FOR UPDATE
  USING (false);

-- DELETE: System only
CREATE POLICY "Only system can delete activity"
